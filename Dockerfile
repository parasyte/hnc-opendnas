...
RUN ln -fs /usr/share/zoneinfo/America/Los_Angeles /etc/localtime && \
    set -x && \
    apt-get -qq update && \
    apt-get -qq -yy install >>/dev/null \
        bc \
        unzip \
        make \
        gcc \
...
RUN cd /root/build && \
    url='https://www.php.net/distributions/php-7.0.15.tar.gz' ; \
    file=$(basename "$url") ; \
    curl -sSL "$url" -o "$file" && \
    tar -zxf "$file" && \
    rm "$file" && \
...
COPY scripts/ /
RUN cd /tmp/ && \
    curl -sSL 'https://github.com/parasyte/OpenDNAS/archive/master.zip' -o 'OpenDNAS.zip' && \
    unzip OpenDNAS.zip && \
    rm OpenDNAS.zip && \
    mv OpenDNAS-master/ /var/www/OpenDNAS/ && \
    echo "Installing OpenDNAS ..." && \
